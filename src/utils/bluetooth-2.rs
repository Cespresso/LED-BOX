...
pub fn init() -> Result<Bluetooth2, Box<dyn std::error::Error>> {

    let nvs = EspDefaultNvsPartition::take()?;
    let mut nvs_hander = EspNvs::new(nvs, "TEST-2", true)? ;
    ...
    // Create a service with custom UUID (modified with -2 suffix concept)
    let my_service = server.create_service(uuid128!("9b574847-f706-436c-bed7-fc01eb0965c2"));

    // Create a characteristic to associate with created service (modified UUID)
    ...
    let init_value = match nvs_hander.get_raw("KEY_NUM-2",&mut buf )? {
    ...
    my_service_characteristic.lock().on_write(move |value|{
        nvs_hander.set_raw("KEY_NUM-2", value.recv_data()).unwrap();
